...
for pol in pollist:
    ...
    for p in polpaths.keys():
        ...
        table_vals[p][pol]= np.round(np.mean(np.array(w_ncri)), 3)
        ...
        if k==0:
            ...
            ax[k].plot(times, np.array(nxcorr_t)-np.array(w_ncri),  marker ='o', mfc=mc, mec=mc, mew=2.5, ms=2.5, ls='-', lw=1,  color=lc, alpha=alpha, label=labels[p])
        else:
            #ax[k].plot(times, nxcorr_t-w_ncri,  marker ='o', mfc=mc, mec=mc, mew=2.5, ms=2.5, ls='-', lw=1,  color=lc, alpha=alpha)
            ax[k].plot(times, np.array(nxcorr_t)-np.array(w_ncri),  marker ='o', mfc=mc, mec=mc, mew=2.5, ms=2.5, ls='-', lw=1,  color=lc, alpha=alpha)
...
table_vals.rename(index={'I':'Thres (I)'},inplace=True)
table_vals.rename(index={'Q':'Thres (Q)'},inplace=True)
table_vals.rename(index={'U':'Thres (U)'},inplace=True)
